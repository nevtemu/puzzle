...
const checkGameStatus = () => {
    let boxStates = [];
    for (k=0; k<8; k++){boxStates.push(document.getElementById(`box${k}`).getAttribute("name"))}
...
const createBoxes = () => {
    ...
    if (k<2){if (getRandomNumber(1,2)==1){boxStates.push("active")}else{boxStates.push("inactive")}}
        ...
        if (boxStates[k-2]==boxStates[k-1]){
            ...
            if (getRandomNumber(1,2)==1){boxStates.push("active")}
            else{boxStates.push("inactive")}}
        }
    }
    for (i=0; i<8; i++){
